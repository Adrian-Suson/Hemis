...
class Region9InstitutionSeeder extends Seeder
{
    public function run()
    {
        ...
        // Faculty groups based on provided data
        $facultyGroups = [
            'GROUP A1' ,
            'GROUP A2' ,
            'GROUP A3' ,
        ...
        // Institution prefixes and types
        $institutionPrefixes = [
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            // Insert 9 Campuses per Institution
            ...
            // Generate 5-15 Programs per Institution
            $programCount = $faker->numberBetween(5, 15);
            $programCategories = ['IT', 'Education', 'Agriculture', 'Health Sciences', 'Engineering'];
            $programNames = [
                'IT' => ['BS Information Technology', 'BS Computer Science', 'MS Information Systems'],
                'Education' => ['B Elementary Education', 'B Secondary Education', 'MA Education'],
                'Agriculture' => ['BS Agriculture', 'BS Forestry', 'MS Agribusiness'],
            ...
            for ($j = 0; $j < $programCount; $j++) {
                ...
                DB::table('curricular_programs')->insert([
                    ...
                    'program_code' => $faker->unique()->numberBetween(100, 999),
                    'major_name' => $faker->boolean(50) ? $faker->word : null,
                    'major_code' => $faker->boolean(50) ? $faker->numberBetween(200, 299) : null,
                    'category' => $category,
                    'serial' => strtoupper(substr($programName, 0, 3)) . '-' . $faker->numerify('###'),
                    'Year' => '2025',
                    'is_thesis_dissertation_required' => $faker->randomElement(['Yes', 'No']),
                    'program_status' => 'Active',
                    'calendar_use_code' => 'Semestral',
                    'program_normal_length_in_years' => $isGraduate ? 2 : 4,
                    'lab_units' => $labUnits,
                    'lecture_units' => $lectureUnits,
                    'total_units' => $labUnits + $lectureUnits,
                    'tuition_per_unit' => $faker->randomFloat(2, $type === 'Private' ? 600 : 400, $type === 'Private' ? 1000 : 800),
                    'program_fee' => $faker->randomFloat(2, $type === 'Private' ? 2000 : 1000, $type === 'Private' ? 4000 : 3000),
                    'program_type' => $isGraduate ? 'Graduate' : 'Undergraduate',
                    'new_students_freshmen_male' => $maleFreshmen,
                    'new_students_freshmen_female' => $femaleFreshmen,
                    'grand_total' => $maleFreshmen + $femaleFreshmen,
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
            }

            // Generate 20-70 Faculty Members per Institution
            ...
            for ($k = 0; $k < $facultyCount; $k++) {
                $gender = $faker->numberBetween(0, 1); // 0 = Female, 1 = Male
                $highestDegree = $faker->numberBetween(1, 3); // 1 = Bachelors, 2 = Masters, 3 = Doctorate
                $department = $faker->randomElement($departments);
                $facultyGroup = $faker->randomElement(array_keys($facultyGroups));
                $isFullTime = $faker->boolean(70); // 70% chance of being full-time
                $hasPlantilla = $faker->boolean(50); // 50% chance of having plantilla
                ...
                DB::table('faculty_profiles')->insert([
                    'institution_id' => $institutionId,
                    'data_date' => Carbon::now(),
                    'faculty_group' => $facultyGroups[$index], // Use A1, A2, A3, B, C1, C2, C3, D, E
                    ...
                    'is_tenured' => $hasPlantilla && $faker->boolean($type === 'SUC' ? 80 : 60), // Plantilla + higher tenure for SUCs
                    ...
                    ) * ($isFullTime ? 1 : 0.5), // Half salary for non-full-time
