...
const UserDialog = ({
    ...
    useEffect(() => {
        const fetchReportYears = async () => {
            try {
                const response = await axios.get(`${config.API_URL}/report-years`);
                setReportYears(response.data);
            } catch (error) {
                AlertComponent.showAlert("Error fetching report years", "error");
            }
        };
    ...
    useEffect(() => {
        if (editingUser) {
            ...
            setPassword("");
            setSelectedYear(editingUser.report_year || "");
        } else {
            setName("");
            setEmail("");
            setRole("Viewer");
        ...
        try {
            ...
            AlertComponent.showAlert(
                editingUser
                    ...
                    : "User created successfully! Assign institutions via UUID mappings.",
                "success"
            );
            onUserUpdated();
            onClose();
            ...
            <div className="bg-white rounded-lg w-full max-w-md mx-4 sm:mx-auto">
                ...
                <div className="p-4 border-b border-gray-200">
                    ...
                    <p className="text-sm text-gray-500">
                        {editingUser
                            ? "Modify user details"
                            : "Create a new user account. Assign institutions via UUID mappings later."}
                    </p>
                </div>
