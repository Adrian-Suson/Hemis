...
function SucCampuses() {
  ...
  const fetchCampuses = async () => {
    ...
    setError("");
    try {
      const response = await axios.get(`${config.API_URL}/suc-campus`, {
        headers: {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-indigo-50/30">
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {isDetailModalOpen && selectedCampus && (
          <div className="fixed inset-0 bg-black/20 backdrop-blur-sm overflow-y-auto h-full w-full z-50 p-4 sm:p-6">
            <div className="relative mx-auto w-full max-w-5xl">
              <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20">
                {/* Modal Header */}
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-t-2xl p-4 sm:p-6 text-white">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center min-w-0 flex-1 mr-4">
                      <div className="h-10 w-10 sm:h-12 sm:w-12 bg-white/20 rounded-xl flex items-center justify-center mr-3 sm:mr-4 flex-shrink-0">
                        <Building2 className="w-5 h-5 sm:w-6 sm:h-6" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <h3 className="text-lg sm:text-xl font-bold truncate">{selectedCampus.name}</h3>
                        <p className="text-blue-100 text-sm hidden sm:block">Campus Details & Information</p>
                      </div>
                    </div>
                    <button
                      onClick={closeDetailModal}
                      className="text-white/80 hover:text-white hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-white/20 rounded-lg p-1.5 sm:p-2 transition-all duration-200 flex-shrink-0"
                    >
