import { Award, MoreHorizontal, Eye, Edit, Trash } from "lucide-react";
import Popper from "../../../../../Components/Popper";
import PropTypes from "prop-types";

function ResearchTb5Table({ data, onView, onEdit, onDelete }) {
    return (
                        ...
                        >
                            <td className="px-6 py-4">
                                <div className="flex items-center">
                                    <div className="flex-shrink-0 h-10 w-10">
                                        <div className="h-10 w-10 rounded-lg flex items-center justify-center bg-gradient-to-br from-amber-100 to-amber-200 text-amber-700">
                                            <Award className="w-5 h-5" />
                                        </div>
                                    </div>
                                    <div className="ml-4">
                                        <div className="text-sm font-medium text-gray-900">
                                            {research.title_of_research_output_award}
                                        </div>
                                        <div className="text-sm text-gray-500">
                                            {research.year_published_accepted_presented_received}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td className="px-6 py-4">
                                <div className="text-sm text-gray-900">
                                    {research.name_of_researcher}
                                </div>
                            </td>
                            <td className="px-6 py-4">
                                <div className="text-sm text-gray-900">
                                    {research.publisher_conference_organizer_confering_body}
                                </div>
                            </td>
...
ResearchTb5Table.propTypes = {
    data: PropTypes.arrayOf(
        PropTypes.shape({
            id: PropTypes.number.isRequired,
            title_of_research_output_award: PropTypes.string.isRequired,
            year_published_accepted_presented_received: PropTypes.string.isRequired,
            name_of_researcher: PropTypes.string.isRequired,
            publisher_conference_organizer_confering_body: PropTypes.string.isRequired
        })
    ).isRequired,
...
export default ResearchTb5Table;