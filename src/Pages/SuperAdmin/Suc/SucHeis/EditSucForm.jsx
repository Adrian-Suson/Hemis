...
function EditSucForm({ initialData, onSave, onCancel, loading = false }) {
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      try {
        // Check if record already exists for this HEI and year (excluding current record)
        ...
        const duplicateRecord = existingRecords.find(record => 
          ...
          record.id !== initialData?.id // Exclude current record
        );
        ...
        AlertComponent.showAlert('SUC institution updated successfully!', 'success');
      } catch (error) {
        AlertComponent.showAlert(error.message || 'Failed to update SUC institution. Please try again.', 'error');
      }
    } else {
