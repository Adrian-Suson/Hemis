...
function AddLucForm({ isOpen, onSave, onCancel, loading = false }) {
    ...
    const fetchHeis = async () => {
        ...
        try {
            ...
            const heisData = Array.isArray(response.data)
                ? response.data
                : response.data.data
                ? response.data.data
                : response.data.heis
            ...
            setErrors((prev) => ({
                ...prev,
                heis: "Failed to load institutions. Please try again.",
            }));
            setHeis([]);
        } finally {
    ...
    const validateForm = () => {
        ...
        if (
            ...
            (isNaN(formData.year_established) ||
        ...
        if (
            ...
            (isNaN(formData.report_year) ||
                formData.report_year < 1800 ||
                formData.report_year > currentYear + 1)
        ) {
    ...
    const uiidOptions = Array.isArray(heis)
        ? heis.map((hei) => ({
              value: hei.uiid,
    ...
    const handleMunicipalityChange = (selectedOption) => {
        ...
        const postalCode = selectedOption
            ? getMunicipalityPostalCode(municipalityName)
            : "";
        ...
        if (selectedOption) {
            ...
            if (selectedMunicipality) {
                const correspondingProvince = provinces.find(
                    (p) => p.id === selectedMunicipality.province_id
                );
                if (correspondingProvince) {
                    selectedProvinceName = correspondingProvince.name;
                    const correspondingRegion = regions.find(
                        (r) => r.id === correspondingProvince.region_id
                    );
    ...
    const handleSubmit = async () => {
        if (validateForm()) {
            try {
                ...
                AlertComponent.showAlert(
                    "LUC institution created successfully!",
                    "success"
                );
            } catch (error) {
                AlertComponent.showAlert(
                    error.message ||
                        "Failed to create LUC institution. Please try again.",
                    "error"
                );
            }
        } else {
                    ...
                    <div className="space-y-3">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                Select LUC Institution{" "}
                                <span className="text-red-500">*</span>
                            </label>
                            {loadingHeis ? (
                                <div className="flex items-center justify-center p-4 border border-gray-200 rounded-lg bg-gray-50">
                                    <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-green-600 mr-2"></div>
                                ...
                                <>
                                    <Select
                                        ...
                                        placeholder="Select an institution (e.g., University of Makati)"
                                        isClearable
                                        isSearchable
                                        className={`text-sm ${
                                            errors.hei_uiid
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                ...
                                <div>
                                    ...
                                    <Select
                                        ...
                                        isSearchable
                                        isDisabled={
                                            !formData.region || locationLoading
                                        }
                                        className="text-sm"
                                        classNamePrefix="select"
                                        styles={customSelectStyles}
